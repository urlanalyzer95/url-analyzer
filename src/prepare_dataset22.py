...
def update_dataset():
    ...
    # 🛡️ Создаём бэкап перед изменениями
    ...
    for idx, row in new_df.iterrows():
        ...
        try:
            ...
            if set(feats_df.columns) != set(feature_cols):
                print(f"   ⚠️ Пропущен {url[:40]}... (несовпадение признаков)")
                continue
    ...
    if not all_features:
        print("⚠️ Нет валидных примеров для добавления")
        return
    ...
    new_features_df['label'] = valid_df['label'].values
