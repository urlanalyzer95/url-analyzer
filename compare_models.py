...
# Имена признаков ДОЛЖНЫ ТОЧНО СОВПАДАТЬ с колонками в датасете
FEATURE_NAMES = [
    'url_length', 'num_dots', 'num_hyphens', 'num_slashes', 'num_params',
    'has_ip', 'has_https', 'has_login', 'has_verify', 'has_account',
    'has_admin', 'has_cp.php', 'is_shortened', 'domain_length'
]
...
def extract_features(url):
    ...
    for word in suspicious:
        # Преобразуем имя колонки: заменяем точку на точку (оставляем как есть)
        key = f'has_{word}'
...
print(f"Датасет загружен: {X.shape}")
